...
ByteStreamExpectationViolation::ByteStreamExpectationViolation(
    const std::string &msg)
    : std::runtime_error(msg) {}

template <typename T>
ByteStreamExpectationViolation
ByteStreamExpectationViolation::property(const std::string &property_name,
                                         const T &expected, const T &actual) {
  return ByteStreamExpectationViolation(
...
std::string ByteStreamExpectation::description() const {
...
std::string ByteStreamAction::description() const {
  return "description missing";
}
...
ByteStreamTestHarness::ByteStreamTestHarness(const std::string &test_name,
...
std::string Write::description() const {
...
std::string InputEnded::description() const {
...
void InputEnded::execute(ByteStream &bs) const {
  ...
  if (input_ended != _input_ended) {
    throw ByteStreamExpectationViolation::property("input_ended", _input_ended,
                                                   input_ended);
  }
}

// BufferEmpty
BufferEmpty::BufferEmpty(const bool buffer_empty)
    : _buffer_empty(buffer_empty) {}
std::string BufferEmpty::description() const {
...
void BufferEmpty::execute(ByteStream &bs) const {
  ...
  if (buffer_empty != _buffer_empty) {
    throw ByteStreamExpectationViolation::property("buffer_empty",
                                                   _buffer_empty, buffer_empty);
  }
}
...
std::string BufferSize::description() const {
...
void BufferSize::execute(ByteStream &bs) const {
  ...
  if (buffer_size != _buffer_size) {
    throw ByteStreamExpectationViolation::property("buffer_size", _buffer_size,
                                                   buffer_size);
  }
}
...
// BytesWritten
BytesWritten::BytesWritten(const size_t bytes_written)
    : _bytes_written(bytes_written) {}
...
std::string BytesRead::description() const {
...
void BytesRead::execute(ByteStream &bs) const {
  ...
  if (bytes_read != _bytes_read) {
    throw ByteStreamExpectationViolation::property("bytes_read", _bytes_read,
                                                   bytes_read);
  }
}
...
std::string Peek::description() const {
...
void Peek::execute(ByteStream &bs) const {
  ...
  if (output != _output) {
    throw ByteStreamExpectationViolation(
        "Expected \"" + _output +
        "\" at the front of the stream, but found \"" + output + "\"");
  }
}
